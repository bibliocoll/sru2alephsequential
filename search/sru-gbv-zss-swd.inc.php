<?php
/* Abfrage wie sru-gbv-swd.inc.php, aber mit pica.iss statt pica.isb (ISSN)
...
$x = "http://sru.gbv.de/gvk?version=1.1&operation=searchRetrieve&query=pica.iss%3D%22".trim($_GET['isbn'])."&maximumRecords=10&recordSchema=picaxml";
...
foreach ( $record as $item ) {
	...
	$pica005 = myget("//m:datafield[@tag='005A']/m:subfield[@code='A' or @code='0']",$xpath);
...
$swd_array =array();	
	...
	$pica[] = array( 'titel' => $pica021A, 'isbn' => $pica005, 'swd' => $swd_array, 'year' => $pica011, 'ppn' => $pica_ppn );
	...
	if (!empty($item['swd'])) {
		...
		/* } */
				...
				foreach ($item['swd'] as $swd) {
					...
					if (!is_array($swd['schlagwort']) && !empty($swd['unterfeld']) && ($swd['unterfeld']!="f")) { // UF f rausschmeissen f. Zs.
...
$file = "data/gbv/swd/gbv-zss-swd.raw.txt";
